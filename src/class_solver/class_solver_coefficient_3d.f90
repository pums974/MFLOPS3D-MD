...
!
  ...
  if (bct==1) then
     A(1,1)=0 ; A(1,2)=0 ; A(1,3)=0 ; A(1,4)=1 ; A(1,5)=1 ; A(1,6)=1 ; A(1,7)=1
     A(1,8)=1 ; A(1,9)=1
     ...
     A(9,1)=-h2**6/720.0 ; A(9,2)=-(-h3-h2)**6/720.0 ; A(9,3)=-(-h4-h3-h2)**6/720.0
     A(9,4)=h1**8/40320.0 ; A(9,5)=0 ; A(9,6)=h2**8/40320.0 ; A(9,7)=(-h3-h2)**8/40320.0
     A(9,8)=(-h4-h3-h2)**8/40320.0 ; A(9,9)=(-h5-h4-h3-h2)**8/40320.0

  elseif(bct==2) then

     A(1,1)=0 ; A(1,2)=0 ; A(1,3)=0 ; A(1,4)=0 ; A(1,5)=1 ; A(1,6)=1 ; A(1,7)=1
     A(1,8)=1 ; A(1,9)=1

     A(2,1)=0 ; A(2,2)=0 ; A(2,3)=0 ; A(2,4)=1 ; A(2,5)=0 ; A(2,6)=h2
     A(2,7)=h3+h2 ; A(2,8)=h4+h3+h2 ; A(2,9)=h5+h4+h3+h2

     A(3,1)=-1 ; A(3,2)=-1 ; A(3,3)=-1 ; A(3,4)=-h1 ; A(3,5)=0 ; A(3,6)=h2**2/2.0
     A(3,7)=(-h3-h2)**2/2.0 ; A(3,8)=(-h4-h3-h2)**2/2.0 ; A(3,9)=(-h5-h4-h3-h2)**2/2.0

     A(4,1)=-h2 ; A(4,2)=-h3-h2 ; A(4,3)=-h4-h3-h2 ; A(4,4)=h1**2/2.0 ; A(4,5)=0 ; A(4,6)=h2**3/6.0
     A(4,7)=-(-h3-h2)**3/6.0 ; A(4,8)=-(-h4-h3-h2)**3/6.0 ; A(4,9)=-(-h5-h4-h3-h2)**3/6.0

     A(5,1)=-h2**2/2.0 ; A(5,2)=-(-h3-h2)**2/2.0 ; A(5,3)=-(-h4-h3-h2)**2/2.0 ; A(5,4)=-h1**3/6.0
     A(5,5)=0 ; A(5,6)=h2**4/24.0 ; A(5,7)=(-h3-h2)**4/24.0 ; A(5,8)=(-h4-h3-h2)**4/24.0
     A(5,9)=(-h5-h4-h3-h2)**4/24.0

     A(6,1)=-h2**3/6.0 ; A(6,2)=(-h3-h2)**3/6.0 ; A(6,3)=(-h4-h3-h2)**3/6.0 ; A(6,4)=h1**4/24.0
     A(6,5)=0 ; A(6,6)=h2**5/120.0 ; A(6,7)=-(-h3-h2)**5/120.0 ; A(6,8)=-(-h4-h3-h2)**5/120.0
     A(6,9)=-(-h5-h4-h3-h2)**5/120.0

     A(7,1)=-h2**4/24.0 ; A(7,2)=-(-h3-h2)**4/24.0 ; A(7,3)=-(-h4-h3-h2)**4/24.0 ; A(7,4)=-h1**5/120.0
     A(7,5)=0 ; A(7,6)=h2**6/720.0 ; A(7,7)=(-h3-h2)**6/720.0 ; A(7,8)=(-h4-h3-h2)**6/720.0
     A(7,9)=(-h5-h4-h3-h2)**6/720.0

     A(8,1)=-h2**5/120.0 ; A(8,2)=(-h3-h2)**5/120.0 ; A(8,3)=(-h4-h3-h2)**5/120.0 ; A(8,4)=h1**6/720.0
     A(8,5)=0 ; A(8,6)=h2**7/5040.0 ; A(8,7)=-(-h3-h2)**7/5040.0 ; A(8,8)=-(-h4-h3-h2)**7/5040.0
     A(8,9)=-(-h5-h4-h3-h2)**7/5040.0

     A(9,1)=-h2**6/720.0 ; A(9,2)=-(-h3-h2)**6/720.0 ; A(9,3)=-(-h4-h3-h2)**6/720.0
     A(9,4)=-h1**7/5040.0 ; A(9,5)=0 ; A(9,6)=h2**8/40320.0 ; A(9,7)=(-h3-h2)**8/40320.0
     A(9,8)=(-h4-h3-h2)**8/40320.0 ; A(9,9)=(-h5-h4-h3-h2)**8/40320.0
...
!  write(333,'(9es17.8,i4,4es17.8)')alpha,beta,gamma,a1,b1,c1,d1,e1,f1,&
!       info,rcond,ferr,berr,work(1)
...
!
  ...
  elseif(bct==2) then
     ...
     A(1,1)=0 ; A(1,2)=0 ; A(1,3)=1 ; A(1,4)=1 ; A(1,5)=1 ; A(1,6)=1 ; 

     A(2,1)=0 ; A(2,2)=1 ; A(2,3)=-h2 ; A(2,4)=0 ; A(2,5)=h3 ; A(2,6)=h4+h3

     A(3,1)=-1 ; A(3,2)=-h2-h1 ; A(3,3)=h2**2/2.0 ; A(3,4)=0 ; A(3,5)=h3**2/2.0
     A(3,6)=(-h4-h3)**2/2.0

     A(4,1)=h2 ; A(4,2)=(h2+h1)**2/2.0 ; A(4,3)=-h2**3/6.0 ; A(4,4)=0 ; A(4,5)=h3**3/6.0
     A(4,6)=-(-h4-h3)**3/6.0

     A(5,1)=-h2**2/2.0 ; A(5,2)=-(h2+h1)**3/6.0 ; A(5,3)=h2**4/24.0 ; A(5,4)=0
     A(5,5)=h3**4/24.0 ; A(5,6)=(-h4-h3)**4/24.0

     A(6,1)=h2**3/6.0 ; A(6,2)=(h2+h1)**4/24.0 ; A(6,3)=-h2**5/120.0 ; A(6,4)=0
     A(6,5)=h3**5/120.0 ; A(6,6)=-(-h4-h3)**5/120.0
...
subroutine der_coeffs_i1(a1,b1,c1,d1,e1,f1,g1,alpha,h1,h2,h3,h4,h5,h6)
! -----------------------------------------------------------------------
...
!
  ...
  A(2,1) = 0 ; A(2,2) = h1 ; A(2,3) = h2+h1 ; A(2,4) = h3+h2+h1 ; A(2,5) = h4+h3+h2+h1
  A(2,6) = h5+h4+h3+h2+h1 ; A(2,7) = h6+h5+h4+h3+h2+h1

  A(3,1) = 0 ; A(3,2) = h1**2/2.0 ; A(3,3) = (-h2-h1)**2/2.0 ; A(3,4) = (-h3-h2-h1)**2/2.0
  A(3,5) = (-h4-h3-h2-h1)**2/2.0 ; A(3,6) = (-h5-h4-h3-h2-h1)**2/2.0
  A(3,7) = (-h6-h5-h4-h3-h2-h1)**2/2.0

  A(4,1) = 0 ; A(4,2) = h1**3/6.0 ; A(4,3) = -(-h2-h1)**3/6.0 ; A(4,4) = -(-h3-h2-h1)**3/6.0
  A(4,5) = -(-h4-h3-h2-h1)**3/6.0 ; A(4,6) = -(-h5-h4-h3-h2-h1)**3/6.0
  A(4,7) = -(-h6-h5-h4-h3-h2-h1)**3/6.0

  A(5,1) = 0 ; A(5,2) = h1**4/24.0 ; A(5,3) = (-h2-h1)**4/24.0 ; A(5,4) = (-h3-h2-h1)**4/24.0
  A(5,5) = (-h4-h3-h2-h1)**4/24.0 ; A(5,6) = (-h5-h4-h3-h2-h1)**4/24.0
  A(5,7) = (-h6-h5-h4-h3-h2-h1)**4/24.0

  A(6,1) = 0 ; A(6,2) = h1**5/120.0 ; A(6,3) = -(-h2-h1)**5/120.0 
  A(6,4) = -(-h3-h2-h1)**5/120.0 ; A(6,5) = -(-h4-h3-h2-h1)**5/120.0
  A(6,6) = -(-h5-h4-h3-h2-h1)**5/120.0 ; A(6,7) = -(-h6-h5-h4-h3-h2-h1)**5/120.0

  A(7,1) = 0 ; A(7,2) = h1**6/720.0 ; A(7,3) = (-h2-h1)**6/720.0
  A(7,4) = (-h3-h2-h1)**6/720.0 ; A(7,5) = (-h4-h3-h2-h1)**6/720.0
  A(7,6) = (-h5-h4-h3-h2-h1)**6/720.0 ; A(7,7) = (-h6-h5-h4-h3-h2-h1)**6/720.0
