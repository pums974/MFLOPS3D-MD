...
subroutine dder_coeffs_i3a(a1,b1,c1,d1,e1,alpha,beta,gamma,delta,h1,h2,h3,h4)
! -----------------------------------------------------------------------
...
!
  ...
  A(1,8)=1 ; A(2,1)=0 ; A(2,2)=0 ; A(2,3)=0 ; A(2,4)=-h2-h1 ; A(2,5)=-h2
  A(2,6)=0 ; A(2,7)=h3 ; A(2,8)=h4+h3

  A(3,1)=-1 ; A(3,2)=-1 ; A(3,3)=-1 ; A(3,4)=(h2+h1)**2/2.0 ; A(3,5)=h2**2/2.0
  A(3,6)=0 ; A(3,7)=h3**2/2.0 ; A(3,8)=(-h4-h3)**2/2.0

  A(4,1)=h2 ; A(4,2)=-h3 ; A(4,3)=-h4-h3 ; A(4,4)=-(h2+h1)**3/6.0 ; A(4,5)=-h2**3/6.0
  A(4,6)=0 ; A(4,7)=h3**3/6.0 ; A(4,8)=-(-h4-h3)**3/6.0

  A(5,1)=-h2**2/2.0 ; A(5,2)=-h3**2/2.0 ; A(5,3)=-(-h4-h3)**2/2.0 ; A(5,4)=(h2+h1)**4/24.0
  A(5,5)=h2**4/24.0 ; A(5,6)=0 ; A(5,7)=h3**4/24.0 ; A(5,8)=(-h4-h3)**4/24.0

  A(6,1)=h2**3/6.0 ; A(6,2)=-h3**3/6.0 ; A(6,3)=(-h4-h3)**3/6.0 ; A(6,4)=-(h2+h1)**5/120.0
  A(6,5)=-h2**5/120.0 ; A(6,6)=0 ; A(6,7)=h3**5/120.0 ; A(6,8)=-(-h4-h3)**5/120.0

  A(7,1)=-h2**4/24.0 ; A(7,2)=-h3**4/24.0 ; A(7,3)=-(-h4-h3)**4/24.0 ; A(7,4)=(h2+h1)**6/720.0
  A(7,5)=h2**6/720.0 ; A(7,6)=0 ; A(7,7)=h3**6/720.0 ; A(7,8)=(-h4-h3)**6/720.0

  A(8,1)=h2**5/120.0 ; A(8,2)=-h3**5/120.0 ; A(8,3)=(-h4-h3)**5/120.0 
  A(8,4)=-(h2+h1)**7/5040.0 ; A(8,5)=-h2**7/5040.0 ; A(8,6)=0
  A(8,7)=h3**7/5040.0 ; A(8,8)=-(-h4-h3)**7/5040.0
  ...
  !-> output coefficient
  alpha=1._rk ; beta=x(1) ; gamma=x(2)  
  a1=x(3) ; b1=x(4) ; c1=x(5) ; d1=x(6) ; e1=x(7) 
...
end subroutine dder_coeffs_i3a
